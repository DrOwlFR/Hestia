...
export async function backupCollection<T extends Document>(Model: Model<T>, collectionName: string, logChannel: TextChannel) {
	try {
		...
		const data = EJSON.stringify(docs, { relaxed: false });
...
export async function weeklyDBBackup(User: Model<dbUser>, LinkedUser: Model<linkedUser>, logChannel: TextChannel) {
	await Promise.all([
