			...
			if (member.roles.cache.has(config.confirmedUserRoleId)) {
				await button.update({
					content: stripIndent(`
						...
						-# ${config.emojis.check} Votre compte Discord a été déconnecté de votre compte du site. Le rôle ${guild?.roles.cache.get(config.confirmedUserRoleId)}, ainsi que vos autres rôles d'accès, vous ont été retirés.
						`),
					components: [],
				});
			} else {
				await button.update({
					content: stripIndent(`
						> *Hestia vous adresse un regard triste. Elle tamponne votre formulaire de départ et vous laisse quitter le Manoir, sans un mot.*\n
						-# ${config.emojis.check} Votre compte Discord a été déconnecté de votre compte du site. Le rôle ${guild?.roles.cache.get(config.nonConfirmedUserRoleId)}, ainsi que vos autres rôles d'accès, vous ont été retirés.
						`),
					components: [],
				});
			}
